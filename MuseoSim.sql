...
-- 1.Obtén todos los datos de las obras de la sala 1 y de la sala 3.
use bdmuseo2021;

select * 
from obras

-- where codsala in(1,3);

 where codsala =1 or codsala = 3;

-- 2.Prepara una consulta que muestre en la primera columna el nombre de la obra y el autor de la misma
-- entre paréntesis (por ejemplo “El Guernica (Picasso)” y, en la segunda y tercera columnas,
-- la sala y la valoración de dicha obra respectivamente. Queremos ver las obras más valoradas primero. Además los encabezados de las columnas deben tener un buen aspecto.
-- uso del parentesis con el concat
select concat(obras.nombreobra,'(',artistas.nomartista,')') as 'Obra y Autor',
		obras.codsala as 'sala',
        obras.valoracion as 'valoracion'
		from obras join artistas on obras.codartista = artistas.codartista
        
        order by obras.valoracion desc;
        
  -- 3.Prepara un procedimiento que muestre, de la 
  -- forma más eficiente posible, los nombres de las
  -- obras restauradas entre dos fechas dadas, así como el
  -- restaurador (su nombre) que realizó el trabajo.  
  
delimiter $$

drop procedure if exists ejercicio3sim $$
create procedure  ejercicio3sim (in fechaInicio date, in fechaFin date) 
begin 
 select obras.nombreobra as 'Obra', 
		restauraciones.fecprevrest as 'Fecha de Restauracion',
        restaurador.nomres
	from obras join restauraciones on obras.codobra = restauraciones.codobra
    join restaurador on restauraciones.codrestaurador = restaurador.codrestaurador
    where restauraciones.fecinirest between fechaInicio and fechaFin;
end $$

delimiter ;
-- faltan poner las fechas pero funciona
call ejercicio3sim();
...
create procedure  ejercicio4 (in nombreObra varchar(100), out autor varchar(100),out valoracion int) 
begin 
	select artistas.nomartista as 'Autor', obras.valoracion as 'Valoracion' 
	into autor, valoracion	
	from  obras left join artistas on obras.codartista = artistas.codartista
...
call ejercicio4('LA MASÏA',@autor,@valoracion);
...
create procedure  ejercicio5 (in fechaInicio date, in fechafin date) 
begin 
	select obras.nombreobra as 'Obra', artistas.nomartista
	
	from  obras join artistas on obras.codartista = artistas.codartista
    
    where obras.codobra in(
		select codobra
        from restauraciones
        where fecinirest  between fechaInicio and fechaFin
...
call ejercicio5('2016-01-01','2021-01-01');
...
begin
    select nomsala AS 'Sala', nombreobra AS 'Obra', IFNULL(valoracion, 'Desconocido,sin valorar') AS 'Valor'
    from salas join obras on salas.codsala = obras.codsala
    order by salas.nomsala, obras.nombreobra;
end $$
